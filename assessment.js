...
for (var prop in snake) {
	if(snake[prop] === snake.sliters || snake[prop] === snake.says || snake[prop] === snake.smells) {
		snake[prop] = "s";
	}
}
...
var firstHero = heroes.splice(heroes.indexOf("superman"), 1).join("");
...
var thirdHero = heroes.filter(function(e) {return (e === "batman")}).join("");
...
function removeItem(arr, str) {
	var newArr = arr.filter(function(e) {return e !== str});
	return newArr;
}
...
function doubleTheFun(thingy) {
	if(Number(thingy) + Number(thingy) === Number(thingy) * 2) {
		dThingy = Number(thingy) * 2;
		return dThingy;
	}
	var dThingy = thingy + thingy;
	return dThingy;
}
...
function getValueOfProperty(obje, prop) {
	return obje[prop];
}
...
function outside(temp, humidity, cloudiness) {
	...
	if (temp < 40 && cloudiness > 60) return "I have icecicles";
	if (temp > 80 && humidity < 40 && cloudiness < 20) return "I'm literally in the desert";
	if (temp > 80 || humidity > 50 || cloudiness > 50) return "Hmm, probably not";
